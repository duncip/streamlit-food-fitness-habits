...
@st.cache
def load_data():
    data = pd.read_csv('nutrition - sample.csv')
...
with intro_container:
    st.title('Most recent data entries:')
    st.write(records_df.tail(7))

st.text('')
st.text('')

trends = st.expander(label = 'Trends')
with trends:
    ...
    with row1_1:
        ...
        fig = Figure()
        ax = fig.subplots()
        sns.lineplot(x=records_df['date'],y=records_df['weight'], color='blue', ax=ax, markers=True)
            
        #ax.set_ylabel('Weight')
        ax.set(ylim=(0, max(records_df['weight'])+5), ylabel = 'Weight', xlabel = '')
        ax.set_xticklabels(records_df['date'], rotation = 45)
        st.pyplot(fig)
